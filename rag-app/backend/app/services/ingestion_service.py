...
async def run_ingestion(
    ...
    async with _ingestion_lock:
        await _run_ingestion_locked(doc_id, file_hash, stored_path, original_filename, settings, registry, store)
...
async def _run_ingestion_locked(
    ...
    try:
        ...
        step_start = time.monotonic()
        dense_vectors = await asyncio.to_thread(embed_passages, settings.embedding_model_name, texts)
        logger.info("embedded %s: %d dense vectors (%.1fs)", original_filename, len(dense_vectors), time.monotonic() - step_start)

        step_start = time.monotonic()
        sparse_vectors = await asyncio.to_thread(embed_sparse, settings.sparse_model_name, texts)
        ...
        record = await asyncio.to_thread(registry.get_by_hash, file_hash)
        if record is None:
        ...
        record.error = None
        await asyncio.to_thread(registry.upsert, record)
        logger.info(
